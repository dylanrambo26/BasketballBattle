...
namespace Network_Multiplayer
{
    public class BallMovementNetwork : NetworkBehaviour
    {
        ...
        private void OnTriggerEnter2D(Collider2D col)
        {
            ...
            else if (col.gameObject.CompareTag("leftPlayerScoreTrigger"))
            {
                gameControllerScript.AddScore(0);
                StartCoroutine(BallDropThroughNet(0));
            }
            else if (col.gameObject.CompareTag("rightPlayerScoreTrigger"))
            {
                gameControllerScript.AddScore(1);
                StartCoroutine(BallDropThroughNet(1));
            }
        }
